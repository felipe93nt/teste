/**
 ****************** Implementação do RSA para Javascript ******************
 */
function encrypt(text) {
	/**
	 * publicKey
	 * 
	 * Propósito: Configura uma chave pública para o RSA
	 * 
	 * @param string encryptionExponent, expoente da chave pública
	 * @param string modulus, módulo do par de chaves
	 * @param string maxDigits, tamanho máximo de bloco
	 */
	var publicKey = function(encryptionExponent, modulus, maxDigits) {
	 ...
	 * rsaEncrypt
	 * 
	 * Propósito: Criptografa uma string com o RSA
	 * 
	 * @param string string, texto a ser criptografado
	 * @param publicKey pubKey, chave pública do RSA
	 */
	function rsaEncrypt(string, pubKey) {
		var charSum = 0;
		...
		while (encrypt.length % pubKey.chunkSize !== 0) {
			encrypt[j++] = 0;
		}
		...
		function encryption(encryptObject) {
			...
			function encryptChar() {
				block = new BigInt();
				j = 0;
				for ( var k = charCounter; k < charCounter + pubKey.chunkSize; ++j) {
					block.digits[j] = encryptObject[k++];
					block.digits[j] += encryptObject[k++] << 8;
				}
				var crypt = pubKey.barrett.powMod(block, pubKey.e);
				var text = pubKey.radix == 16 ? biToHex(crypt) : biToString(crypt,
						pubKey.radix);
				encrypted += text + " ";
				charCounter += pubKey.chunkSize;
				if (charCounter < encryptObject.length) {
					return encryptChar();
				} else {
					var encryptedString = encrypted.substring(0,
							encrypted.length - 1);
	...
	/**
	 * Implementação do BigInteger para Javascript
	 * 
	 * @author David Shapiro, Daniel Griesser
	 */
	var biRadixBase = 2;
	var biRadixBits = 16;
	...
	function biFromHex(s) {
		...
		for ( var i = sl, j = 0; i > 0; i -= 4, ++j) {
			result.digits[j] = hexToDigit(s.substr(Math.max(i - 4, 0), Math.min(i,
					4)));
		}
		return result;
	}
	...
	var modulus = "";
	var maxDigits = "131";
	
	/** Configura a chave pública **/
	var key = new publicKey(encryptionExponent, modulus, maxDigits);
	
	/** Criptografa o texto **/
	var ct = rsaEncrypt(text, key);
