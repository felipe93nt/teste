package br.com.bradesco.web.ricc.view.bean.questionariolargecorporate.bean;
...
@SessionScoped
@Named(value = "questionarioLargeCorporateBean")
public class QuestionarioLargeCorporateBean extends AbstractBean<QuestionarioLargeCorporateViewHelper> {
	...
	public String iniciarPagina() {
		setViewHelper(new QuestionarioLargeCorporateViewHelper());
		...
		this.viewHelper.getModel().setCodSegmentoRelatorio(
				String.valueOf(SegmentoEnum.LARGE_CORPORATE.getCodigo()));
		try {
			carregarCopiaLista();
		...
		return RiccConstants.TELA_QUESTIONARIO_LARGE_CORPORATE;
	}
	...
	public void incluirAdministrativos() {
		if (validarCamposAdministrativos()) {
			QuestionarioLargeCorporateVO model = getViewHelper()
					.getModel();
	 ...
	 * 
	 * @name alterarSetor
	 * @see QuestionarioLargeCorporateBean
	 * @return
	 * @return String
	 */
	public String alterarSetor() {
		try {
			...
			String retorno = "";
			String paginaAnterior = RiccConstants.TELA_QUESTIONARIO_LARGE_CORPORATE;
		...
		for (EndividamentoBancarioVO itemTabela : viewHelper.getModel().getListaEndividamento()) {
			if (!itemTabela.equals(viewHelper.getModel().getCopiaListaEndividamento().get(contador))) {
				...
				//EXCLUSÃO
					...
					try {
						QuestionarioLargeCorporateVO model = getViewHelper().getModel();
						segmentoService.manutencaoEndividamentoBancario(model,
			...
			if (!itemTabela.equals(viewHelper.getModel()
				...
				if (Integer.parseInt(itemTabela.getNumSequencial()) == 0
					...
					// ALTERAÇÃO
						...
						.getNumSequencial()) {
						try {
							QuestionarioLargeCorporateVO model = viewHelper
									.getModel();
