...
import br.com.bradesco.web.ricc.view.bean.treinamento.equipes.equipe1.viewhelper.Equipe1ConsultaQuestionarioImobiliarioViewHelper;

/**
 * @name Equipe1ConsultaQuestionarioImobiliarioBean
 * @author Equipe 1 - André Nunes, Isaac Maciel, João Carlos e João Gil
 * @see Equipe1ConsultaQuestionarioImobiliarioViewHelper.java
 * @version 1.0
 * @since 11/05/2020
 */
@SessionScoped
@Named(value = "equipe1ConsultaQuestionarioImobiliarioBean")
public class Equipe1ConsultaQuestionarioImobiliarioBean extends AbstractBean<Equipe1ConsultaQuestionarioImobiliarioViewHelper> {
	...
	public String iniciarPagina() {
		setViewHelper(new Equipe1ConsultaQuestionarioImobiliarioViewHelper());
		viewHelper.inicializar();
		
		carregarDadosCadastro();
		carregarDados();
		
		return RiccConstants.TREINAMENTO_EQUIPE1_CONSULTA_CORPORATE_IMOBILIARIO;
	}
	
	private void carregarDadosCadastro() {
		viewHelper.getModel().setCodGrupoEconomico(codGrupoEconomico);
		viewHelper.getModel().setDescGrupoEconomico(descGrupoEconomico);
		viewHelper.getModel().setCodRelatorio(codRelatorio);
	}
