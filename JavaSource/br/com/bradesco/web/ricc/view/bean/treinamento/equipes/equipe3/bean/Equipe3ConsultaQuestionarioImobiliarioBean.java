...
import br.com.bradesco.web.ricc.view.bean.treinamento.equipes.equipe3.viewhelper.Equipe3ConsultaQuestionarioImobiliarioViewHelper;

@SessionScoped
@Named(value = "equipe3ConsultaQuestionarioImobiliarioBean")
public class Equipe3ConsultaQuestionarioImobiliarioBean extends
AbstractBean<Equipe3ConsultaQuestionarioImobiliarioViewHelper> {
	...
	public String iniciarPagina() {
		setViewHelper(new Equipe3ConsultaQuestionarioImobiliarioViewHelper());
		viewHelper.inicializar();
		carregarDadosCadastro();
		carregarDados();
		return "equipe3.consulta.questionario.imobiliario.iniciar";
	}

	private void carregarDadosCadastro() {
		Equipe3Bean bean1 = (Equipe3Bean) FacesUtils
				.getManagedBean("equipe3Bean");
		Equipe3ConsultaQuestionarioAltoVarejoBean bean2 = (Equipe3ConsultaQuestionarioAltoVarejoBean) FacesUtils
				.getManagedBean("equipe3ConsultaQuestionarioAltoVarejoBean");
		viewHelper.getModel().setCodGrupoEconomico(
				bean1.getViewHelper().getModelPesquisa().getNumGrupoLabel());
		viewHelper.getModel().setDescGrupoEconomico(
				bean1.getViewHelper().getModelPesquisa().getNomeGrupoLabel());
		viewHelper.getModel().setCodRelatorio(
				String.valueOf(bean2.getViewHelper().getModel()
						.getCodRelatorio()));
	}
	...
	public void listarAreaAtuacao() {
		...
		AbaQuestionarioImobiliarioVO questionario = viewHelper.getModel()
				.getQuestionario();
		...
		try {
			questionario.setListaAreaAtuacao(questionarioImobiliarioService
					.consultaAreaAtuacao(viewHelper.getModel()));
		} catch (Exception e) {
			siteUtil.trataExceptionRecebida(e);
			return;
		}
	...
	public void listarLandBank() {
		...
		try {
			questionario.setListaLandBank(questionarioImobiliarioService
					.consultaLandBank(viewHelper.getModel(), totalTerreno,
							totalProjetada));
		} catch (Exception e) {
			siteUtil.trataExceptionRecebida(e);
			return;
	...
	public void listarObrasAndamento() {
		...
		try {
			QuestionarioImobiliarioVO retorno = questionarioImobiliarioService
					.consultaObrasAndamento(viewHelper.getModel(), sb);
	...
	public void listarObrasConcluidas() {
		...
		try {
			QuestionarioImobiliarioVO retorno = questionarioImobiliarioService
					.consultaObrasConcluidas(viewHelper.getModel(), sb);
