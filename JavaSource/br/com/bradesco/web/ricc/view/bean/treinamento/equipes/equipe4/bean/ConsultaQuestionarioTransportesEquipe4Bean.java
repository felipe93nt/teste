...
import br.com.bradesco.web.ricc.view.bean.treinamento.equipes.equipe4.viewhelper.ConsultaQuestionarioTransportesEquipe4ViewHelper;

@SessionScoped
@Named(value = "consultaQuestionarioTransportesEquipe4Bean")
public class ConsultaQuestionarioTransportesEquipe4Bean extends AbstractBean<ConsultaQuestionarioTransportesEquipe4ViewHelper> {
	...
	public String iniciarPagina() {
		setViewHelper(new ConsultaQuestionarioTransportesEquipe4ViewHelper());
		viewHelper.inicializar(); 
		carregarDadosCadastro(); 
		
		
		return RiccConstants.TELA_EQUIPE4BEAN_CONSULTA_QUESTIONARIO_TRANSPORTE;
	}
	
	private void carregarDadosCadastro() {
		Equipe4Bean bean = (Equipe4Bean) FacesUtils.getManagedBean(RiccConstants.BEAN_CADASTRAMENTO_EQUIPE4BEAN_RIC);
		viewHelper.getModel().setCodGrupo(
				bean.getViewHelper().getModel().getCodGrupo());
		viewHelper.getModel().setNomeGrupo(
				bean.getViewHelper().getModel().getNomeGrupo());
		viewHelper.getModel().setCodRelatorio(
				String.valueOf(bean.getViewHelper().getModel()
						.getCodRelatorio()));
	}
...
//		calculoGrauAlienacao();
//		calculoGrauLeasing();
//		calculoGrauLeasingCagiro();
		...
		/*
		 * consulta comentários - RICCIAA9
		 * Tipo 15
		 * 	1 – Detalhe Situação dos Principais Contratos.
			2 – Licitações e seus Vencimentos.
			3 – Outorga.
	...
	public void calculoGrauAlienacao(){
		DadosTransportesDatabaseVO dadosTransportes = getViewHelper().getModel().getDadosTransportesDatabase();
		BigDecimal saldoDevedor = siteUtil.valueToMainframeBigDecimal(dadosTransportes.getSaldoDevedorFinameLeasing());
		BigDecimal valorFrota = siteUtil.valueToMainframeBigDecimal(dadosTransportes.getValorFrota());
		
		if(!siteUtil.valorVazio(valorFrota.toPlainString())){
			//Multiplica por 100 para trazer pra %
			BigDecimal calculo = saldoDevedor.divide(valorFrota, 2, BigDecimal.ROUND_HALF_DOWN).multiply(new BigDecimal(100));
			dadosTransportes.setPrcGrauAlienacaoFrota(calculo.setScale(2, BigDecimal.ROUND_HALF_DOWN).toPlainString());
		} else {
	...
	public void calculoGrauLeasing(){
		DadosTransportesDatabaseVO dadosTransportes = getViewHelper().getModel().getDadosTransportesDatabase();
		BigDecimal desembolso = siteUtil.valueToMainframeBigDecimal(dadosTransportes.getDesembolsoMensalFinameLeasing());
		BigDecimal faturamento = siteUtil.valueToMainframeBigDecimal(dadosTransportes.getFaturamentoMedioMensal());
		
		if(!siteUtil.valorVazio(faturamento.toPlainString())){
			//Multiplica por 100 para trazer pra %
			BigDecimal calculo = desembolso.divide(faturamento, 2, BigDecimal.ROUND_HALF_DOWN).multiply(new BigDecimal(100));
			dadosTransportes.setPrcGrauComprometimentoReceitasComFinameLeasing(calculo.toPlainString());
		} else {
			dadosTransportes.setPrcGrauComprometimentoReceitasComFinameLeasing("");
		}
	}
	
	public void calculoGrauLeasingCagiro(){
		DadosTransportesDatabaseVO dadosTransportes = getViewHelper().getModel().getDadosTransportesDatabase();
		BigDecimal desembolsoFiname = siteUtil.valueToMainframeBigDecimal(dadosTransportes.getDesembolsoMensalFinameLeasing());
		BigDecimal desembolsoCagiro = siteUtil.valueToMainframeBigDecimal(dadosTransportes.getDesembolsoMensalCagiro());
		BigDecimal faturamento = siteUtil.valueToMainframeBigDecimal(dadosTransportes.getFaturamentoMedioMensal());
		
		if(!siteUtil.valorVazio(faturamento.toPlainString())){
			//Multiplica por 100 para trazer pra %
			BigDecimal calculo = (desembolsoFiname.add(desembolsoCagiro)).divide(faturamento, 2, BigDecimal.ROUND_HALF_DOWN).multiply(new BigDecimal(100));
			dadosTransportes.setPrcGrauComprometimentoReceitasComFinameLeasingCagiro(calculo.toPlainString());
		} else {
			dadosTransportes.setPrcGrauComprometimentoReceitasComFinameLeasingCagiro("");
		}
	}
	...
	private void consultaDadosSetorTransporte() {
		...
		try {
			model.setDadosTransportesDatabase(questionarioTransportesService.consultaDadosSetorTransportes(model));
		} catch (Exception e) {
	...
	private void consultaAberturaFaturamentoEbitda() {
		QuestionarioTransportesVO model = viewHelper.getModel();
		try {
			model.setFaturamentoEbitda(questionarioTransportesService.consultaAberturaFaturamentoEmpresa(model));
		} catch (Exception e) {
			siteUtil.trataExceptionRecebida(e);
		}
	}
	...
	private void consultaPrincipaisCustosOperacionais() {
		...
		try {
			_lista = questionarioTransportesService.consultaCustosOperacionais(model);
			
			PrincipaisCustosOperacionaisVO _principaisCustosOperacionais = model.getPrincipaisCustosOperacionais();
			_principaisCustosOperacionais.setCustosOperacionaisAno1(_lista.get(0));
			_principaisCustosOperacionais.setCustosOperacionaisAno2(_lista.get(1));
			_principaisCustosOperacionais.setCustosOperacionaisAno3(_lista.get(2));
			_principaisCustosOperacionais.setCustosOperacionaisAnoExpectativa(_lista.get(3));
	...
	private void consultaTiposContratos() {
		...
		try {
			_lista = questionarioTransportesService.consultaTiposContratos(model);
	...
	private void consultaTransporteCargas() {
		...
		try {
			_lista = questionarioTransportesService.consultaTransportesCargas(model);
	...
	private void calculaTotalTransporteCargas(int ano){
		TransportesCargasVO _transporte = getViewHelper().getTransporteCargasAno(ano);
		
		BigDecimal _propria = siteUtil.valueToMainframeBigDecimal(_transporte.getPrcPropria());
		BigDecimal _agregados = siteUtil.valueToMainframeBigDecimal(_transporte.getPrcAgregados());
		BigDecimal _terceiros = siteUtil.valueToMainframeBigDecimal(_transporte.getPrcTerceiros());
	...
	private void calculaTotalFaturamentoLocadoras(int ano){
		ValoresLocadorasVO _vlrsLocadoras = getViewHelper().getValoresLocadorasAno(ano);
		
		BigDecimal _locacao = siteUtil.valueToMainframeBigDecimal(_vlrsLocadoras.getPrcLocacao());
		BigDecimal _vendaVeiculos = siteUtil.valueToMainframeBigDecimal(_vlrsLocadoras.getPrcVendaVeiculos());
		BigDecimal _outros = siteUtil.valueToMainframeBigDecimal(_vlrsLocadoras.getPrcOutros());
	...
	private void consultaLocadoras() {
		QuestionarioTransportesVO model = viewHelper.getModel();
		try {
			model.setLocadoras(questionarioTransportesService.consultaLocadoras(model));
		} catch (Exception e) {
			siteUtil.trataExceptionRecebida(e);
		}
	}
	...
	private void consultaTransportePassageiros() {
		...
		try {
			_lista = questionarioTransportesService.consultaTransportesPassageiros(model);
	...
	private void consultaComentarios() {
		...
		try {
			...
			model.getFaturamentoEbitda().setDescSituacaoPrincipaisContratos(_comentarios.get(0).getComentario());
			model.getFaturamentoEbitda().setDescLicitacoesVencimentos(_comentarios.get(1).getComentario());
			model.getFaturamentoEbitda().setDescOutorgaUltimosValoresFonteFinanciamento(_comentarios.get(2).getComentario());
			model.getPrincipaisCustosOperacionais().setDescPrincipaisInvestimentos(_comentarios.get(3).getComentario());
	...
	public void atribuirValorAnos(AjaxBehaviorEvent e){
		CustosOperacionaisVO _custosOperacionais = viewHelper.getCustosOperacionaisAno(1);
