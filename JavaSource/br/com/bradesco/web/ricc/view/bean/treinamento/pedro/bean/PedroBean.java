...
import br.com.bradesco.web.ricc.view.bean.treinamento.pedro.viewhelper.PedroViewHelper;
 ...
 */
@SessionScoped
@Named(value = "pedroBean")
public class PedroBean extends AbstractBean<PedroViewHelper> {
	/**
	 *
	 */
	...
	public String iniciarPagina() {
		this.viewHelper = new PedroViewHelper();
		this.viewHelper.inicializar();
		return RiccConstants.TELA_TREINAMENTO_PEDRO_CADASTRO;
	}
	...
	public String salvar() {
		if (validaSalvar()) {
			...
			this.viewHelper.getListaClientesSalvosVO().add(clienteVO);
			this.viewHelper.setListaClienteVO(this.viewHelper.getListaClientesSalvosVO());
			exibirTableClientes();
	...
	public void buscaPorCpf() {
		String cpfPesquisa = viewHelper.getCpfPesquisa();
		List<ClientePedroVO> listaClientesCpfVO = new ArrayList<ClientePedroVO>();
		for (ClientePedroVO c : viewHelper.getListaClientesSalvosVO()) {
			if (cpfPesquisa.equals(c.getCpf())) {
				listaClientesCpfVO.add(c);
			}
		}
		if (listaClientesCpfVO.isEmpty()) {
			viewHelper.setListaClienteVO(this.viewHelper.getListaClientesSalvosVO());
			BradescoFacesUtils.addInfoModalMessage(
					"Nenhum registro encontrado", Boolean.FALSE);
		} else {
			viewHelper.setListaClienteVO(listaClientesCpfVO);
			viewHelper.setCpfPesquisa(VAZIO);
		}
	}
	...
	public String consultar() {
		...
		paginaAnterior = RiccConstants.TELA_TREINAMENTO_PEDRO_CADASTRO;
		this.viewHelper.setBtnConsultar(Boolean.FALSE);
		return RiccConstants.TELA_TREINAMENTO_PEDRO_CONSULTA;
	}
