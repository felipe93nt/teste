...
import br.com.bradesco.web.ricc.view.bean.treinamento.equipes.equipe4.viewhelper.Equipe4ViewHelper;
 ...
 */
@SessionScoped
@Named(value = "equipe4Bean")
public class Equipe4Bean extends
		AbstractBean<Equipe4ViewHelper> {
	...
	public String iniciarPagina() {
		this.viewHelper = new Equipe4ViewHelper();
		this.viewHelper.inicializar();
		carregarDados();
		
		return RiccConstants.TELA_EQUIPE4BEAN_CONSULTA_QUESTIONARIO_LARGE_CORPORATE;
	}
	
	private void carregarDados() {

		if (siteUtil.isNotEmptyOrNull(viewHelper.getModel())) {
			consultaComentarios();
			consultarSegmento();
			listarQuadroAtivos();
			listarQuadroAdm();
	...
	private void consultaComentarios() {
		QuestionarioLargeCorporateVO model = viewHelper.getModel();
	...
	private void listarQuadroAtivos() {
		QuestionarioLargeCorporateVO model = this.viewHelper.getModel();
	...
	public String consultarSetor() {
		...
		try {
			...
			String paginaAnterior = RiccConstants.TELA_EQUIPE4BEAN_CONSULTA_QUESTIONARIO_LARGE_CORPORATE;
			
			if (SetorEnum.AGRONEGOCIO.getCodigo().equals(codigoSetor)) {
				ConsultaQuestionarioAgronegocioEquipe4Bean bean = (ConsultaQuestionarioAgronegocioEquipe4Bean) FacesUtils
						.getManagedBean(RiccConstants.BEAN_CONSULTA_QUESTIONARIO_AGRONEGOCIO_EQUIPE4);
				...
				ConsultaQuestionarioImobiliarioEquipe4Bean bean = (ConsultaQuestionarioImobiliarioEquipe4Bean) FacesUtils
						.getManagedBean(RiccConstants.BEAN_CONSULTA_QUESTIONARIO_IMOBILIARIO_EQUIPE4);
				retorno = bean.iniciarPagina();
				bean.setPaginaAnterior(paginaAnterior);
				...
				ConsultaQuestionarioProjetoEquipe4Bean bean = (ConsultaQuestionarioProjetoEquipe4Bean) FacesUtils
						.getManagedBean(RiccConstants.BEAN_CONSULTA_QUESTIONARIO_PROJETOS_EQUIPE4);
				retorno = bean.iniciarPagina();
				bean.setPaginaAnterior(paginaAnterior);
				bean.getViewHelper().getModel().setCodGrupo(model.getCodGrupo());
				bean.getViewHelper().getModel().setCodSegmentoRelatorio(model.getCodSegmentoRelatorio());
				...
				ConsultaQuestionarioTransportesEquipe4Bean bean = (ConsultaQuestionarioTransportesEquipe4Bean) 
						FacesUtils.getManagedBean(RiccConstants.BEAN_CONSULTA_QUESTIONARIO_TRANSPORTES_EQUIPE4);
				retorno = bean.iniciarPagina();
				bean.setPaginaAnterior(paginaAnterior);
				bean.getViewHelper().getModel().setCodGrupo(model.getCodGrupo());
				bean.getViewHelper().getModel().setCodSegmentoRelatorio(model.getCodSegmentoRelatorio());
